...
export async function POST(req: NextRequest) {
  ...
  const existing = await sql`SELECT id, user_id FROM people WHERE LOWER(name) = LOWER(${name}) LIMIT 1`
  let personId: number
  if (existing.length > 0) {
    ...
    const [p] = await sql`
      INSERT INTO people (name, relation, photo, user_id)
      VALUES (${name}, ${relation}, ${photo}, ${userId})
      RETURNING id
    `
  ...
  const rows = await sql`
    INSERT INTO condolences (name, relation, message, photo, person_id, user_id, moderation)
    VALUES (${name}, ${relation}, ${message}, ${photo}, ${personId}, ${userId}, ${moderation})
    RETURNING *
  `
