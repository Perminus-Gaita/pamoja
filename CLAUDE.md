...
- Footer: tagline "Pamoja — together. A free, open-source digital condolence book and memorial." above the nav links About / FAQ / Terms & Conditions / Contact / GitHub ★ <count> / Support. The last two come from **`FooterMeta`** (`components/footer-meta.tsx`, rendered INSIDE `.dir-foot-links` in both directory + doc-shell, styled like the sibling links): "GitHub ★ N" links the repo (count omitted on API failure), "Support" links GitHub Sponsors. Star count comes from `lib/github.ts` `getRepoStars()` — server-side fetch with `next: { revalidate: 3600 }` (ISR, one GitHub API call/hour). `app/directory/page.tsx` and `DocShell` are async and pass `stars` down (Directory takes a `stars` prop since it's a client component). `.github/FUNDING.yml` (`github: [Perminus-Gaita]`) enables the repo Sponsor button. (The spec's star-ask sentence + "free & open source" line were dropped by owner request 19 July — footer links only.)
